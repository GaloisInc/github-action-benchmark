...
function findAlerts(curSuite, prevSuite, threshold) {
    ...
    for (const series in curSuite.benches) {
        for (const current of curSuite.benches[series]) {
            const prev = ((_a = prevSuite.benches[series]) !== null && _a !== void 0 ? _a : []).find((b) => b.name === current.name);
            if (prev === undefined) {
...
function buildComment(benchName, curSuite, prevSuite, expandableDetails = true) {
    ...
    for (const series in curSuite.benches) {
        for (const current of curSuite.benches[series]) {
            let line;
            const prev = ((_a = prevSuite.benches[series]) !== null && _a !== void 0 ? _a : []).find((i) => i.name === current.name);
            if (prev) {
                const ratio = getRatio(curSuite.tool, prev, current);
